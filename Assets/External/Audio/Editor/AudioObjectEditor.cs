...
public class AudioObjectEditor : Editor
{
    ...
    private static bool _channelsShown = false;
    private static bool _soundsShown = false;
    private static bool _mixerShown = false;
    private static Dictionary<Sound, bool> _soundFoldoutStates;
    private static Dictionary<Channel, bool> _mixerFoldoutStates;
    ...
    private void OnEnable()
    {
        ...
        FillFoldoutStateDictionaries();
        Undo.undoRedoPerformed += FillFoldoutStateDictionaries;
    ...
    private void ShowChannels()
    {
        ...
        for (int i = 0; i < _audio.Channels.Count; i++)
        {
            ...
            _audio.Channels[i].EditorColor = EditorGUILayout.ColorField(_audio.Channels[i].EditorColor, GUILayout.Width(60));
        ...
        {
            var newChannel = new Channel("New");
            _audio.Channels.Add(new Channel("New"));
            _mixerFoldoutStates.Add(newChannel, false);
        }
    ...
    private void ShowSounds()
    {
        ...
        foreach (var channel in _audio.Channels)
        {
            ...
            {
                var colorRect = new Rect(vertical.rect.x - 5, vertical.rect.y, 5, vertical.rect.height);
                var boxStyle = new GUIStyle(GUI.skin.box);
                boxStyle.normal.background = TextureFromColor(channel.EditorColor);
                GUI.Box(colorRect, "", boxStyle);

                EditorGUI.indentLevel++;
                for (int i = 0; i < channel.Sounds.Count; i++)
                {
                    using (var soundVertical = new EditorGUILayout.VerticalScope("Box"))
                    {
                        CreateDropArea(soundVertical.rect, sound: channel.Sounds[i]);
    ...
    private bool ShowSoundEditor(Sound sound)
    {
        ...
        _soundFoldoutStates[sound] = EditorGUILayout.Foldout(_soundFoldoutStates[sound], soundName, true);
        GUI.color = Color.white;
        if (GUILayout.Button(EditorGUIUtility.IconContent("d_winbtn_win_close"), GUILayout.Width(30))) return false;
        GUILayout.EndHorizontal();

        if (!_soundFoldoutStates[sound]) return true;
        EditorGUI.indentLevel++;
        GUILayout.Space(2);
    ...
    private void FillFoldoutStateDictionaries()
    {
        if (_soundFoldoutStates == null) _soundFoldoutStates = new Dictionary<Sound, bool>();
        if (_mixerFoldoutStates == null) _mixerFoldoutStates = new Dictionary<Channel, bool>();

        foreach (var channel in _audio.Channels)
        {
            _mixerFoldoutStates.TryAdd(channel, false);
            foreach (var sound in channel.Sounds)
            {
                _soundFoldoutStates.TryAdd(sound, false);
            }
        }
    }
    ...
    private void ShowMixer()
    {
        foreach (var channel in _audio.Channels)
        {
            ...
            {
                var colorRect = new Rect(vertical.rect.x, vertical.rect.y, 5, vertical.rect.height);
                var boxStyle = new GUIStyle(GUI.skin.box);
                boxStyle.normal.background = TextureFromColor(channel.EditorColor);
                GUI.Box(colorRect, "", boxStyle);
                ...
                GUILayout.BeginHorizontal();
                GUILayout.Space(20);
                _mixerFoldoutStates[channel] = EditorGUILayout.Foldout(_mixerFoldoutStates[channel], channel.Name, true);
                GUILayout.EndHorizontal();
                if (!_mixerFoldoutStates[channel]) continue;
    ...
    private void AddSound(Channel channel, Sound sound = null)
    {
        ...
        _soundFoldoutStates.Add(sound, false);
    }
    ...
    private static Texture2D TextureFromColor(Color color)
    {
        Color[] pix = new Color[1] { color };
